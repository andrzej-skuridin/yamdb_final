...
class TestDockerfileCompose:
    ...
    def test_docker_compose_file(self):
        try:
            ...
            assert (
                False
            ), f'Проверьте, что в директорию {infra_dir_path} добавлен файл `docker-compose.yaml`'

        assert re.search(
            r'image:\s+postgres:', docker_compose
        ), 'Проверьте, что  в файл docker-compose.yaml добавлен образ postgres:latest'
        assert re.search(
            r'image:\s+([a-zA-Z0-9]+)\/([a-zA-Z0-9_\.])+(\:[a-zA-Z0-9_-]+)?',
            docker_compose,
